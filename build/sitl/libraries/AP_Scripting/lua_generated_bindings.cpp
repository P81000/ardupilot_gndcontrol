...
int new_camera_state_t(lua_State *L) {
    luaL_checkstack(L, 2, "Out of stack");
    void *ud = lua_newuserdata(L, sizeof(camera_state_t));
    new (ud) camera_state_t();
    luaL_getmetatable(L, "camera_state_t");
    lua_setmetatable(L, -2);
    return 1;
}
...
#if (AP_CAMERA_SCRIPTING_ENABLED == 1)
camera_state_t * check_camera_state_t(lua_State *L, int arg) {
    void *data = luaL_checkudata(L, arg, "camera_state_t");
    return (camera_state_t *)data;
}
#endif // (AP_CAMERA_SCRIPTING_ENABLED == 1)
...
static int AP_Camera_get_state(lua_State *L) {
    ...
    if (data) {
        new_camera_state_t(L);
        *check_camera_state_t(L, -1) = data_5003;
        return 1;
    }
    return 0;
}
...
#endif // (AP_EFI_SCRIPTING_ENABLED == 1)
#if (AP_CAMERA_SCRIPTING_ENABLED == 1)
static int camera_state_t_auto_focus(lua_State *L) {
    camera_state_t *ud = check_camera_state_t(L, 1);
    binding_argcheck(L, 1);
    lua_pushinteger(L, ud->auto_focus);
    return 1;
}

static int camera_state_t_focus_step(lua_State *L) {
    camera_state_t *ud = check_camera_state_t(L, 1);
    binding_argcheck(L, 1);
    lua_pushinteger(L, ud->focus_step);
    return 1;
}

static int camera_state_t_zoom_step(lua_State *L) {
    camera_state_t *ud = check_camera_state_t(L, 1);
    binding_argcheck(L, 1);
    lua_pushinteger(L, ud->zoom_step);
    return 1;
}

static int camera_state_t_recording_video(lua_State *L) {
    camera_state_t *ud = check_camera_state_t(L, 1);
    binding_argcheck(L, 1);
...
static int camera_state_t_take_pic_incr(lua_State *L) {
    camera_state_t *ud = check_camera_state_t(L, 1);
    binding_argcheck(L, 1);
...
#if (AP_CAMERA_SCRIPTING_ENABLED == 1)
const luaL_Reg camera_state_t_meta[] = {
    {"auto_focus", camera_state_t_auto_focus},
    {"focus_step", camera_state_t_focus_step},
    {"zoom_step", camera_state_t_zoom_step},
    {"recording_video", camera_state_t_recording_video},
    {"take_pic_incr", camera_state_t_take_pic_incr},
};

static int camera_state_t_index(lua_State *L) {
    const char * name = luaL_checkstring(L, 2);
    if (load_function(L,camera_state_t_meta,ARRAY_SIZE(camera_state_t_meta),name)) {
        return 1;
    }
    return 0;
}
...
#endif // (AP_EFI_SCRIPTING_ENABLED == 1)
#if (AP_CAMERA_SCRIPTING_ENABLED == 1)
    {"camera_state_t", camera_state_t_index, nullptr},
#endif // (AP_CAMERA_SCRIPTING_ENABLED == 1)
#if HAL_MAX_CAN_PROTOCOL_DRIVERS
...
#endif // (AP_EFI_SCRIPTING_ENABLED == 1)
#if (AP_CAMERA_SCRIPTING_ENABLED == 1)
    {"camera_state_t", new_camera_state_t},
#endif // (AP_CAMERA_SCRIPTING_ENABLED == 1)
#if HAL_MAX_CAN_PROTOCOL_DRIVERS
